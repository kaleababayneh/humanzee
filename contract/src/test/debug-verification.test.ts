...
describe("Debug Schnorr Signature Verification", () => {
  it("should debug step-by-step verification process", () => {
    ...
    // Step 6: Test posting without verification (should work)
    console.log("🔍 Testing post without verification...");
    try {
      const ledgerState = simulator.post("Test message", "kaleab", credential);
      console.log(`✅ Post succeeded: sequence=${ledgerState.sequence}, posts=${ledgerState.posts.length()}`);
    } catch (error) {
